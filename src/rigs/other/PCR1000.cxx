...
void RIG_PCR1000::initialize()
{
	...
	get_trace(1, "check_power");
	wait_crlf(cmd = check_power_command, "check ON/OFF");
//	gett("");
	...
	if (replystr.rfind("H100") != std::string::npos) {
		...
		sendCommand(cmd = power_on_command);
		sett("");
	}
...
{
	// Turn off the radio
	set_trace(1, "shutdown()");
	wait_crlf(cmd = power_off_command, "power OFF");
	sett("");
	RigSerial->Baud(9600);
}
//----------------------------------------------------------------------
...
std::string RIG_PCR1000::strFreqModeBW(XCVR_STATE &freqMode)
{
//                          0         1
//                          012345678901234567 8 9
	std::string freq_cmd = "K00072000000000000\r\n";
//                          K0GMMMKKKHHHmmff00    
//                          | |          | | | 
//                          | |          | | |__________ 00\r\n\0 terminator
//                          | |          | |____________ Bandwidth
//                          | |          |               00=2.8kHz, 01=6kHz, 
//                          | |          |               02=15kHz, 03=50kHz, 04=230kHz
//                          | |          |______________ Mode code, 
//                          | |                          00=LSB, 01=USB, 02=AM, 
//                          | |                          03=CW,  05=NFM, 06=WFM, 04 is NOT USED!
//                          | |_________________________ 10 digit frequency in Hertz
//                          |___________________________ command K0
	...
	// Set the mode
	freq_cmd[13] = mode_chr[ freqMode.imode ];

	// Set the band width
	freq_cmd[15] = '0'+ freqMode.iBW ;
...
int RIG_PCR1000::get_smeter() {	 // returns 0-100
	...
	wait_crlf(cmd = get_smeter_command, "smeter");
//	gett("");

	size_t p = replystr.find("I1");
	if (p == std::string::npos) return 0;

	ret = hexTo(replystr[p+2]) * 16 + hexTo(replystr[p+3]) ;
...
void RIG_PCR1000::set_volume_control(int val) // 0 .. 100
{
	...
	wait_crlf(cmd = volume_command, "volume");
	sett("");
}
...
void RIG_PCR1000::set_bwA(int val) {
	...
	bwA = val;
	if (inuse == onA)
		wait_crlf(strFreqModeBW(A), "set_bw   A");
}
...
void RIG_PCR1000::set_bwB(int val) {
	...
	bwB = val;
	if (inuse == onB)
		wait_crlf(strFreqModeBW(B), "set_bw   B");
}
...
void RIG_PCR1000::set_modeA(int val)
{
	...
	A.imode = val ;
	if (inuse == onA)
		wait_crlf(strFreqModeBW(A), "set mode A"); 
}
...
void RIG_PCR1000::set_modeB(int val)
{
	...
	B.imode = val ;
	if (inuse == onB)
		wait_crlf( strFreqModeBW(B), "set_mode B");
}
...
void RIG_PCR1000::selectA() {
	inuse = onA;
	wait_crlf(strFreqModeBW(A), "select A");
}
//----------------------------------------------------------------------
void RIG_PCR1000::selectB() {
	inuse = onB;
	wait_crlf(strFreqModeBW(B), "select B");
}
//----------------------------------------------------------------------
...
void RIG_PCR1000::swapAB() {
	...
	A = T;
	if (inuse == onA)
		wait_crlf(strFreqModeBW(A), "swapAB");
	else
		wait_crlf(strFreqModeBW(B), "swapAB");
}
...
void RIG_PCR1000::set_squelch(int val) {
	...
	wait_crlf(cmd = squelch_command, "squelch");
	sett("");
}
 ...
 * the actual shift. Just divide by 10 and add 0x80 before its set.
 */
void RIG_PCR1000::set_if_shift(int val) {
	...
	wait_crlf(cmd = if_shift_command, "IF shift");
	sett("");
}
...
void RIG_PCR1000::set_attenuator(int val) {
	if( val != attenuator ) {
		if( val ) {
			// Turn att on
			set_trace(1, "set_attenuator ON");
			wait_crlf(cmd = att_on_command, "att ON");
			sett("");
			attenuator = 1 ;
		} else {
			// Turn att off
			set_trace(1, "set_attenuator OFF");
			wait_crlf(cmd = att_off_command, "att OFF");
			sett("");
			attenuator = 0 ;
		}
	}
...
returned as a packet if Autoupdate is ON (See G301) and one of thesmeter
values changes.
...
LD8201 to set to manual. It's actually LD82000 (note three 0's) to turn off,
and can be varied from LD82001 to LD820FF. I don't know exactly what the tracking
thingie does, but it has a pronounced affect turning it off or on. - Editor's
note this has now been corrected in this document, it is the tracking of the
RF stage with frequency I believe.

There's also a mention of H800 in the documents. H8 will accept values from 00 to FF,
and has an odd effect on the audio. Values greater than or equal to 0x81 cause a very
brief (100ms or so) drop out on the audio. Values 0x80 and below don't do this.
Interestingly enough, when G301 is in effect, changing H8 causes an H9 message
to be output. H9 can be read with the H9? command. I've only seen 0x00, 0x01,
and 0x10 be reported back.

The following command sequence seems to have something to do with squelch and
signal strength.

   H800; J4180; H881; H9? (reports H901), J4100.

Now the squelch should be open, but isn't. Setting H800 will open the squelch again.
Now do H8FF; H9? Audio opens, and H9? reports H910. Curious. All my experimenting
is in NBFM mode, so perhaps some of these other registers come into play in other modes.

Something else that's not mentioned in any of the documentation is that when G301
mode is on, commands don't generate a response unless they're interrogative
(i.e. GD? or H1?).
