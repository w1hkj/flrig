...
void read_mode()
{
	...
	pthread_mutex_unlock(&mutex_serial);
	if (vfo.imode != nu_mode) {
		useB ? vfoB.imode = nu_mode : vfoA.imode = nu_mode;
		vfo.imode = nu_mode;
...
void read_bandwidth()
{
	...
	if (vfo.iBW != nu_BW) {
		vfo.iBW = nu_BW;
		useB ? vfoB.iBW = nu_BW : vfoA.iBW = nu_BW;
...
void setBW()
{
	pthread_mutex_lock(&mutex_serial);
		vfo.iBW = opBW->index();
		useB ? selrig->set_bwB(vfo.iBW) : selrig->set_bwA(vfo.iBW);
	pthread_mutex_unlock(&mutex_serial);
	send_bandwidth_changed();
...
void setMode()
{
	...
	vfo.imode = useB ? vfoB.imode = nu_mode : vfoA.imode = nu_mode;

	pthread_mutex_lock(&mutex_serial);
	useB ? selrig->set_modeB(vfo.imode) : selrig->set_modeA(vfo.imode);
	if (selrig->adjust_bandwidth(vfo.imode) != -1)
		useB ? selrig->set_bwB(vfo.iBW) : selrig->set_bwA(vfo.iBW);
	Fl::awake(updateBandwidthControl);
	send_mode_changed();
...
void selectFreq() {
	...
	vfo.freq  = oplist[n].freq;
	vfo.imode = oplist[n].imode;
	vfo.iBW   = oplist[n].iBW;

	if (!useB) {
		vfoA = vfo;
		FreqDispA->value(vfo.freq);
		selrig->set_vfoA(vfo.freq);
...
// disable the serial thread
	...
	if (progStatus.use_rig_data) {
		progStatus.freq = transceiverA.freq;
		progStatus.opMODE = transceiverA.imode;
		progStatus.opBW = transceiverA.iBW;
		mnuKeepData->set();
	} else
		mnuKeepData->clear();
	...
	if (selrig->has_mode_control) {
		...
		updateBandwidthControl();
		selrig->set_modeA(progStatus.opMODE);
	} else {
		opMODE->add(" ");
		opMODE->index(0);
		opMODE->deactivate();
	...
	if (selrig->has_bandwidth_control) {
		...
		selrig->set_bwA(progStatus.opBW);
	} else {
		opBW->add(" ");
		opBW->index(0);
		opBW->deactivate();
