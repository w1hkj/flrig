...
{
	cmd = "RA;";
	if (wait_char(';', 7, 100, "get att", ASC) < 7) return 0;
...
void RIG_TS590SG::set_split(bool val) 
{
	split = val;
	if (useB) {
		if (val) {
...
int RIG_TS590SG::get_volume_control()
{
	cmd = "AG0;";
...
void RIG_TS590SG::set_volume_control(int val) 
{
...
// val 0 .. 100
void RIG_TS590SG::set_mic_gain(int val)
{
	cmd = "MG000;";
	for (int i = 3; i > 0; i--) {
		cmd[1+i] += val % 10;
...
void RIG_TS590SG::get_mic_min_max_step(int &min, int &max, int &step)
{
	min = 0;
	max = 100;
	step = 1;
...
void RIG_TS590SG::set_squelch(int val)
{
	cmd = "SQ0";
...
void RIG_TS590SG::get_squelch_min_max_step(int &min, int &max, int &step)
{
